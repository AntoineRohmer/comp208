...
public class MainFrame extends JFrame {
	...
	public MainFrame() {
		...
		try {
			Vector<String> tableName = DatabaseConnection.getTable(table_name);
			int size = tableName.size();
			for (int i = 0; i < size; i++) {
				open.add(new JMenuItem(tableName.get(i))).addActionListener(new table());
				;
			}
		} catch (Exception e) {

		}
		open.addActionListener(new table());
		/**
		 * final JMenuItem appointments = new JMenuItem("appointments");
		 * appointments.addActionListener(new table()); final JMenuItem customers = new
		 * JMenuItem("customers"); customers.addActionListener(new table()); final
		 * JMenuItem invoice = new JMenuItem("invoice"); invoice.addActionListener(new
		 * table()); final JMenuItem parts = new JMenuItem("parts");
		 * parts.addActionListener(new table()); final JMenuItem repairs = new
		 * JMenuItem("repairs"); repairs.addActionListener(new table()); final JMenuItem
		 * staff = new JMenuItem("staff"); staff.addActionListener(new table()); final
		 * JMenuItem vehicles = new JMenuItem("vehicles");
		 * vehicles.addActionListener(new table()); open.add(appointments);
		 * open.add(customers); open.add(invoice); open.add(parts); open.add(repairs);
		 * open.add(staff); open.add(vehicles);
		 **/
	...
	public MainFrame(Vector<String> columnName, Vector<Vector<String>> columnValue) {
		this.setTitle("Main");
		final JMenuBar menuBar = new JMenuBar();
		setJMenuBar(menuBar);
		final JMenu file = new JMenu("File");
